...
void main() {
  ...
  float dU = (h(vec2(v_uv.x + 1/u_texture_4_size.x, v_uv.y)) - h(vec2(v_uv.x, v_uv.y))) * u_height_scaling * u_normal_scaling;
  float dV = (h(vec2(v_uv.x, v_uv.y + 1/u_texture_4_size.y)) - h(vec2(v_uv.x, v_uv.y))) * u_height_scaling * u_normal_scaling;
